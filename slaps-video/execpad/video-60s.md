# execpad — ~60 second launch video
...
| 0:48–0:52 | npm install execpad |
| 0:54–1:00 | slaps.dev/execpad |
...
**(0:08)** **execpad** runs shell, Python, Node, and SQL **against your project directory**. One API. Four engines.
...
**(0:56)** **execpad** — [slaps.dev/execpad](https://slaps.dev/execpad)
...
2. **0:08–0:18** — Terminal: `npm install execpad` then a 4-line snippet: `import { Runtime } from "execpad"` → `new Runtime("./project")` → `run("python", "...")` (don’t read code aloud; VO carries it).
3. **0:18–0:28** — Browser full-width: [slaps.dev/execpad](https://slaps.dev/execpad) hero; pause on product name.
4. **0:28–0:36** — Same page, scroll to “Languages” or feature grid; or terminal showing `readonly` / `overlay` in comments.
5. **0:36–0:48** — Docs: [slaps.dev/docs/execpad/getting-started](https://slaps.dev/docs/execpad/getting-started) — slow scroll one screen.
6. **0:48–0:54** — Terminal again: `npm install execpad` + `pip install execpad` on two lines (stacked cuts OK).
7. **0:54–1:00** — End card: `slaps.dev/execpad` + small `execpad` wordmark type.
